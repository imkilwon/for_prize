...
class SemiAutoSelectNumberLotteryScreen extends StatefulWidget {
  const SemiAutoSelectNumberLotteryScreen({Key? key}) : super(key: key);
          ...
          child: SafeArea(
            child: Column(
              children: [
                Padding(
                  padding:
                      const EdgeInsets.only(top: 20, left: 14.0, bottom: 4),
                  child: Row(
                    ...
                    children: [
                      ...
                      ElevatedButton(
                          onPressed: () {
                            setState(() {
                              if (numberSet.length + holdNumber.length < 6) {
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    backgroundColor: Colors.white,
                                    duration: Duration(seconds: 1),
                                    content: Text("고정 숫자와 선택 숫자가 합쳐서 6개 이상 이어야 합니다.",style: TextStyle(color: Colors.black),),
                                  ),
                                );
                              } else {
                                tmp.clear();
                              ...
                              if (!holdNumber.contains(i + 1)) {
                                ...
                                ScaffoldMessenger.of(context).showSnackBar(
                                  const SnackBar(
                                    backgroundColor: Colors.white,
                                    duration: Duration(seconds: 1),
                                    content: Text("고정하신 번호를 선택하실 수 없습니다.",style: TextStyle(color: Colors.black),),
                                  ),
                                );
                              }
                            } else {
